        ...
        try:
            ...
            if days_since_registration >= NodeStatistics.DAYS_IN_WEEK:
                check_in_times = self._get_check_in_time_single_day(
                    node_name, start_time.weekday(), start_time.weekday(), start_time, time_range_in_minutes,
                )
                expected_check_in_times = (start_time - node_registration_time).days // 7

                current_chance = 1.0
                for day, actual_checkins in check_in_times.items():
                    for check_in in actual_checkins:
                        if int(check_in[1]) > expected_check_in_times:
                            raise HTTPException(status_code=500, detail="check_in_count > expected_check_in_times")
                ...
                for day, actual_checkins in check_in_times.items():
                    ...
                    for check_in in actual_checkins:
                        if int(check_in[1]) > expected_check_in_times:
                            raise HTTPException(status_code=500, detail="check_in_count > expected_check_in_times")
                        check_in_change_in_day *= int(check_in[1]) / expected_check_in_times
